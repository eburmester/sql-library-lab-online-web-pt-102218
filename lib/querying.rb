...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(character_books.book_id) FROM characters INNER JOIN character_books AT characters.id=character_books.character_id GROUP BY book_id ORDER BY COUNT(book_id) DESC"
end
